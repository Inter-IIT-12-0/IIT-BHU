...
const Notifications = ({ notifs }) => {
    const handleAccept = (notif) => {
        axios.patch(`/api/myInvites/?teamId=${notif.id}`).then(res => window.location.reload()).catch(console.log)
    }
    const handleReject = (notif) => {
        axios.delete(`/api/myInvites/?teamId=${notif.id}`).then(res => window.location.reload()).catch(console.log)
    }
    return (
        <div className='absolute top-12 right-3 bg-sky-100 w-96 h-72 p-3'>
            {
                notifs.map(notif => (
