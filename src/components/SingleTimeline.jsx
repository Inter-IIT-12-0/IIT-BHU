...
const SingleTimeline = ({ milestone }) => {
    return (
        <li className="mb-10 ms-6">
            ...
            <div className={`text-lg font-bold ${milestone.status === 'Not Started' ? 'text-zinc-500 italic' : ''} `}> {milestone.heading} </div>
            <div className="mb-4 text-base font-normal">
                {
                    milestone.status === 'Completed' ?
                        <span> Completed </span> :
                        <div>
                            <span className={`font-normal ${milestone.status === 'In Progress' ? 'text-red-600 italic' : ''} `}>Due date: </span>
